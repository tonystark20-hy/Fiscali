...
const tableData = [
  { 
    ...
    subRows: [
      { rowtablehead: "Subscriber Revenues", millionsofusd: "6,614.0", categorylabels: "Revenues", rowconfidence: "Low" },
      { rowtablehead: "Advertising Revenues", millionsofusd: "1,700.0", categorylabels: "Revenues", rowconfidence: "Medium" },
      { rowtablehead: "Other Revenues", millionsofusd: "151.0", categorylabels: "Revenues", rowconfidence: "High" },
      { rowtablehead: "Equipment Revenues", millionsofusd: "201.0", categorylabels: "Revenues", rowconfidence: "High" },
      { rowtablehead: "Gross Profit", millionsofusd: "3,653", categorylabels: "Revenues", rowconfidence: "High" },
      { rowtablehead: "Total Revenues", millionsofusd: "8696.0",categorylabels: "Total Revenues", rowconfidence: "High"}
    ]
  },
  {
  },
...
export default function ReviewFinancialSpreadsPage() {
  ...
  const handleClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (showTable) {
      ...
      setShowTable(true);
      setTablePosition({ x: event.clientX - left/2, y: event.clientY - top/2});
  ...
  const tableColumns = React.useMemo(() => {
    ...
    return [
        ...
        cell: (info) => (
          ...
          <div className="flex justify-between items-center pd-2">
            <Heading
              ...
              style={{ color: 'black', fontWeight: isMainRow(info.row) || info.getValue?.() === "Total Revenues" ? "bold" : "normal" }}
            >
              {info?.getValue?.()}
            </Heading>
            ...
            {info.row.getCanExpand() && (
              ...
              >
                {info.row.getIsExpanded() ? <img src="images/up-arrow.svg" alt="Expanded" width="10" /> : <img src="images/down-arrow.svg" alt="Collapsed"  width="10" />}
      ...
      tableColumnHelper.accessor("rowconfidence", {
        cell: (info) => (
          <div className="flex md:w-full p-2">
            <Heading
              as="p"
              className={`flex justify-center items-center h-[20px] px-2.5 py-px rounded-[10px] ${
                info?.getValue?.() == 'High' ? 'bg-green-300' :
                info?.getValue?.() == 'Medium' ? 'bg-yellow-200' :
                info?.getValue?.() == 'Low' ? 'bg-red-300' : 'bg-white-500'
              }`}
              style={{ color: `${
                info?.getValue?.() == 'High' ? '#038C8C' :
                info?.getValue?.() == 'Medium' ? '#CCB400' :
                info?.getValue?.() == 'Low' ? '#DF4D5A' : 'white'
              }` }}
            >
            {info?.getValue?.()}
            </Heading>
      ...
      tableColumnHelper.accessor("rowview", {
        cell: (info) => (
          <div className="flex justify-center">
            ...
            <div className="h-[36px] flex items-center justify-center">
              <input type="checkbox" defaultChecked={info.row.original.rowconfidence === "High"}
                style={{ 
        ...
        header: (info) => (
          <div className="flex bg-white-A700">
            <div className="h-[36px] w-[41px] flex items-center justify-center"
            style={{ color: 'black' }}>
            </div>
          </div>
        ),
      ...
      <div className="h-[1024px] w-full bg-white-A700_02 relative flex">
          ...
          >
            ...
            >
              ...
              <div className="flex self-stretch justify-between items-start gap-5 p-1">
                ...
                <Img src="images/img_frame_234.svg" alt="image_two" className="h-[10px] mt-[3px] mr-0.5 md:mr-0" />
              </div>
        ...
        <div className="flex flex-col w-[66%] top-[13%] right-0 left-0 m-auto absolute">
          ...
          <Text size="md" as="p" className="mt-3">
            Please review line items and click on any row to amend. You will need to confirm all unchecked boxes before
            proceeding.{" "}
          </Text>
      ...
      {showTable && (
        ...
        >
          <CategoryRanking initialValue={""} />
            ...
            <Button className="flex items-center justify-center h-[39px] mt-[51px] mr-[30%] ml-[60%] px-[15px] mb-5 md:ml-0 sm:px-5 text-white-A700_01 text-center text-base font-medium bg-indigo-800 rounded-[3px]"
